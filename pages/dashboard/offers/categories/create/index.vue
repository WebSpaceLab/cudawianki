...
const onSubmit = async () => {
    ...
    await $categories.addCategory(form).then((data) => {
        cancel()
    }).catch(err => {
...
async function getOfferCategoriesCreate() {
    await $categories.getOfferCategoriesCreate()
    .then(({data}) => {
        console.log(data)
        ...
        <template #main>
            <div>
                <div class="w-100 h-full container mx-auto p-8 flex flex-col justify-center items-center space-y-5">
                    ...
                    <select v-model="form.offer_id" aria-label="Media type" id="type" class="w-full 0 bg-background-light dark:bg-background-dark rounded-lg text-muted-light dark:text-muted-dark  dark:border-muted-dark shadow-sm lg:text-sm focus:outline-none focus:ring-focus focus:border-focus">
                        <option v-for="offer in allOffers" :key="offer.value" :value="offer.value">{{ offer.label }}</option>
                    </select>
